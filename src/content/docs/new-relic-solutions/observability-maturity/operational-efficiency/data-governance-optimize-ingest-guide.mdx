...
| 1. Use the data ingest governance baseline dashboard to identify an account that's sending a large amount of log data to New Relic.<br/>2. Find and select that account from the account dropdown menu.<br/>3. Navigate to the **logs** page of the account and select **patterns** from the left-side menu.<br/>4. Review the log patterns shown and give some examples of the low value log patterns. What makes them low value? How much total reduction you could achieve by dropping these logs? |
